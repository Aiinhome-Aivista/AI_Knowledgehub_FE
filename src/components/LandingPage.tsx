...
const LandingPage: React.FC<LandingPageProps> = ({ onCardClick }) => {
  ...
  const isNoData = !data || (
    ...
    (!data.most_searched || data.most_searched.length === 0) &&
    (!data.most_written || data.most_written.length === 0) &&
    (!data.data_coverage || (data.data_coverage.total_articles === 0 && data.data_coverage.total_topics === 0 && data.data_coverage.total_connectors === 0))
  );

  if (isNoData) {
    return (
      <div className="p-8 flex flex-col justify-center items-center h-full space-y-4 theme-bg-primary text-center">
        <div className="max-w-md p-6 rounded-2xl border theme-border theme-card-bg space-y-4 shadow-lg animate-in fade-in duration-300">
          ...
          <div>
            <p className="theme-text-primary font-bold text-lg">No Data Found</p>
            <p className="text-xs theme-text-secondary mt-1">No keywords, search queries, or content ingestion summaries are available yet.</p>
          </div>
          <button
            onClick={fetchLandingData}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold cursor-pointer transition-colors shadow-md shadow-indigo-500/10"
          >
            Refresh Telemetry
          </button>
        </div>
